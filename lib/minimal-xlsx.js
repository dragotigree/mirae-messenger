...
function buildStyleRegistry(allRows) {
  ...
  const zebraFillId = addFill('FFF1F5F9');
  ...
  const centerWrap = { h: 'center', v: 'center', wrap: true };
  ...
  const wardRowXf = {};
  const wardRowZebraXf = {};
  const wardColXf = {};
  const wardRowLeftXf = {};
  const wardRowLeftZebraXf = {};
  Object.keys(wardRowFill).forEach((k) => {
    wardRowXf[k] = regXf(0, wardRowFill[k], false, 1, center);
    wardRowZebraXf[k] = regXf(0, zebraFillId, false, 1, center);
    wardColXf[k] = regXf(0, wardColFill[k], false, 1, center);
    wardRowLeftXf[k] = regXf(0, wardRowFill[k], false, 1, leftWrap);
    wardRowLeftZebraXf[k] = regXf(0, zebraFillId, false, 1, leftWrap);
  });
  const doctorXf = {};
  ...
  const resolveCellStyle = (row, colIndex, rowIndex0) => {
    ...
    if (colIndex === COL_RM || colIndex === COL_WARD) {
      return wardColXf[wKey] ?? wardColXf.other ?? xfDefault;
    }
    const left = colIndex === COL_PURPOSE || colIndex === COL_REMARK || colIndex === COL_AUTHOR;
    if (left) {
      if (zebra) return wardRowLeftZebraXf[wKey] ?? wardRowLeftZebraXf.other ?? xfDefault;
      return wardRowLeftXf[wKey] ?? wardRowLeftXf.other ?? xfDefault;
    }
    if (zebra) return wardRowZebraXf[wKey] ?? wardRowZebraXf.other ?? xfDefault;
