...
const SCHEDULE_BOARD_COL_WIDTHS = [9, 10, 8, 12, 9, 26, 32, 11, 11, 14];
...
function buildStyleRegistry(allRows) {
  ...
  addFill('FFFFFFFF');
  addFill('FFBFBFBF');
  // 제목·헤더·메타: 조금 더 차분한 톤 (병동·RM 색은 위 팔레트 그대로)
  const titleFillId = addFill('FF1E3A5F');
  const headerFillId = addFill('FF0F766E');
  const metaFillId = addFill('FFF1F5F9');
  const zebraFillId = addFill('FFF8FAFC');
  const sampleHeaderFill = addFill('FF0E7490');
  const sampleHintFill = addFill('FFECFEFF');
  ...
  const xfHeader = regXf(1, headerFillId, true, 2, center);
  const xfSampleHeader = regXf(1, sampleHeaderFill, true, 2, center);
  ...
  // RM·병동: 기존 색 유지 + 약간 굵은 글씨로 구분 가독성
  const wardColXf = {};
  Object.keys(wardColFill).forEach((k) => {
    wardColXf[k] = regXf(4, wardColFill[k], true, 1, center);
  });
  const rmColXf = {};
  Object.keys(rmColFill).forEach((k) => {
    rmColXf[k] = regXf(4, rmColFill[k], true, 1, center);
  });
  const doctorXf = {};
  ...
  // 0 none · 1 soft grid · 2 header (하단 accent) · 3 title (무테)
  const softBorder = '<left style="thin"><color rgb="FFE2E8F0"/></left><right style="thin"><color rgb="FFE2E8F0"/></right><top style="thin"><color rgb="FFE2E8F0"/></top><bottom style="thin"><color rgb="FFE2E8F0"/></bottom>';
  const headerBorder = '<left style="thin"><color rgb="FF99F6E4"/></left><right style="thin"><color rgb="FF99F6E4"/></right><top style="thin"><color rgb="FF99F6E4"/></top><bottom style="medium"><color rgb="FF134E4A"/></bottom>';
  const bordersXml = [
    '<border><left/><right/><top/><bottom/><diagonal/></border>',
    `<border>${softBorder}<diagonal/></border>`,
    `<border>${headerBorder}<diagonal/></border>`
  ].join('');

  const fontsXml = [
    '<font><sz val="10"/><color rgb="FF334155"/><name val="Malgun Gothic"/><family val="2"/><charset val="129"/></font>',
    '<font><b/><sz val="10"/><color rgb="FFFFFFFF"/><name val="Malgun Gothic"/><family val="2"/><charset val="129"/></font>',
    '<font><b/><sz val="15"/><color rgb="FFFFFFFF"/><name val="Malgun Gothic"/><family val="2"/><charset val="129"/></font>',
    '<font><sz val="9"/><color rgb="FF64748B"/><name val="Malgun Gothic"/><family val="2"/><charset val="129"/></font>',
    '<font><b/><sz val="10"/><color rgb="FF0F172A"/><name val="Malgun Gothic"/><family val="2"/><charset val="129"/></font>'
  ].join('');
...
function writeRowXml(lines, rowIndex, cells, rowObj, styleRegistry, resolveStyle, opts) {
  const o = opts || {};
  let ht = '';
  if (o.headerRow) ht = ' ht="26" customHeight="1"';
  else if (o.titleRow) ht = ' ht="36" customHeight="1"';
  else if (o.metaRow) ht = ' ht="22" customHeight="1"';
  else if (o.autoHeight) ht = ` ht="${Math.max(22, estimateRowHeight(cells))}" customHeight="1"`;
...
/**
 * 보드형 시트: 차분한 제목·헤더 / 건수·추출시각 / 필터 / RM·병동 색 유지
 */
function buildScheduleBoardSheetXml(headers, rows, styleRegistry, sheetMeta) {
  ...
  const dateOffset = hdrs[0] === '일자' ? 1 : 0;
  const colWidths = dateOffset
    ? [12, ...SCHEDULE_BOARD_COL_WIDTHS].slice(0, colCount)
    : SCHEDULE_BOARD_COL_WIDTHS.slice(0, colCount);

  const lines = [];
  lines.push('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>');
  lines.push('<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">');
  lines.push(sheetViewsXml(3));
  lines.push(buildColsXml(colWidths));
  ...
  const resolve = (row, ci, isHeader, dataIndex) => {
    ...
    if (dateOffset && ci === 0) {
      const zebra = ((dataIndex || 0) % 2) === 1;
      return zebra ? styleRegistry.xfZebra : styleRegistry.xfPlain;
    }
    return styleRegistry.resolveCellStyle(row, ci - dateOffset, dataIndex || 0);
  };
...
function buildGuideLines(boardDate) {
  ...
  return [
    ...
    '■ 데이터 시트: 제목·헤더 스타일 · RM·병동 색 구분 · 필터·틀 고정이 적용됩니다.',
    '',
    '구분 예: 외진, 외출, 외박, 입원, 퇴원, 전실, 전원 등 (현황판 구분과 동일)',
...
<sheets>${sheetTags}</sheets>
  ...
  normalized.forEach((sh, i) => {
    ...
    if (sh.kind === 'pasteTemplate') {
      ...
      const sheetHdrs = (sh.headers && sh.headers.length) ? sh.headers : hdrs;
      data = buildScheduleBoardSheetXml(sheetHdrs, sh.rows, styleRegistry, {
        title: sh.title,
