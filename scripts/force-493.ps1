...
Write-Host "=== FORCE 1.0.493 (peer traffic monitor) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce493'
...
$ok = $main.Contains('listPeerTrafficStats') -and $html.Contains('data-master-tab="traffic"') -and $html.Contains('renderMasterPeerTraffic')
Write-Host "version=$ver markers=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.493 installed' -ForegroundColor Green
return
