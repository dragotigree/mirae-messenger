...
Write-Host "=== FORCE 1.0.499 (load-sim) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce499'
...
$ok = $main.Contains('LOADTEST_CMD') -and $main.Contains('runLoadTestCommand') -and $html.Contains('runMasterLoadSimInject')
Write-Host "version=$ver markers=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.499 installed' -ForegroundColor Green
return
