...
Write-Host "=== FORCE 1.0.492 (update freeze fix) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce492'
...
$ok = $main.Contains('isVersionMeta') -and $main.Contains('LARGE_INPLACE_BYTES') -and $html.Contains('compact-rail-expand-btn')
Write-Host "version=$ver markers=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.492 installed' -ForegroundColor Green
return
