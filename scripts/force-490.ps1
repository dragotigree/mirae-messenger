...
Write-Host "=== FORCE 1.0.490 (phone_no migrate + TCP quiet) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce490'
...
$ok = $main.Contains('TCP deferred') -and $main.Contains("ALTER TABLE known_users ADD COLUMN") -and $main.Contains('phone_no TEXT') -and $main.Contains('logDbErr._last')
Write-Host "version=$ver markers=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.490 installed' -ForegroundColor Green
return
