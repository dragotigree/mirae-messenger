...
Write-Host "=== FORCE 1.0.495 (restore + stability) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce495'
...
$ok = $main.Contains('allowUdpReceive') -and $main.Contains('targetIP === MY_IP') -and $html.Contains('search-kw-hit') -and $html.Contains('is-pending')
Write-Host "version=$ver markers=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.495 installed' -ForegroundColor Green
return
