...
\$Ref = '0dd923f7297f35f90fae4ce5e99946aecf73f951'
$Expected = '1.0.488'
...
Write-Host "=== FORCE 1.0.488 (TCP storm / click freeze) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce488'
...
$ok = $main.Contains('NETWORK_QUIET_MS') -and $main.Contains('isSafeUiMode')
Write-Host "version=$ver tcpQuiet=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.488 installed' -ForegroundColor Green
Write-Host 'If still frozen, create SAFE_UI.txt then restart:' -ForegroundColor Yellow
Write-Host '  New-Item "C:\Apps\Mirae Messenger\userdata\SAFE_UI.txt" -ItemType File -Force'
return
