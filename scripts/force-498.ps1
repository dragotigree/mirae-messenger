...
Write-Host "=== FORCE 1.0.498 (unread inbox) ===" -ForegroundColor Cyan
...
$wc.Headers['User-Agent'] = 'MiraeForce498'
...
$ok = $html.Contains('compact-inbox-bar') -and $html.Contains('openFirstUnreadChat') -and $html.Contains('sidebarUnreadOnly')
Write-Host "version=$ver markers=$ok"
if ($ver -ne $Expected -or -not $ok) { throw "verify failed" }
Write-Host 'OK: 1.0.498 installed' -ForegroundColor Green
return
